...
OPENAI_API_KEY=...        # For OpenAI (via sub2API gateway, see below)
OPENAI_BASE_URL=...       # Optional, sub2API gateway address (default: http://localhost:8080)
...
ccm openai            # OpenAI via sub2API gateway (gpt-6-astra)
...
ccc openai            # OpenAI via sub2API gateway
```
...
| OpenAI | `ccm openai` / `ccm gpt` | - | Your sub2API gateway (`OPENAI_BASE_URL`) |
...
### OpenAI (via sub2API Gateway)

OpenAI does not offer an official Anthropic-compatible endpoint, so `ccm openai` routes through a self-hosted [sub2API](https://github.com/Wei-Shaw/sub2api) gateway that manages ChatGPT subscription accounts and converts `/v1/messages` (Anthropic format) to OpenAI upstream.

```bash
# In ~/.ccm_config
OPENAI_BASE_URL=http://your-sub2api-host:8080   # trailing slash or /v1 suffix is auto-stripped
OPENAI_API_KEY=sk-...                           # API key generated by your sub2API platform
OPENAI_MODEL=gpt-6-astra                        # optional override (default: gpt-6-astra)

# Usage
ccm openai            # or: ccm gpt / ccm gpt6
ccc openai            # switch + launch Claude Code
```

The API key must belong to a sub2API group whose platform is OpenAI; sub2API routes `/v1/messages` by group platform automatically. All model slots (Sonnet/Opus/Haiku/Subagent) use `OPENAI_MODEL`.

> ⚠️ **Terms of Service risk**: relaying subscription accounts through sub2API may violate upstream providers' ToS. Use at your own risk.
